...
export function AdminNoteForm({
  ...
  const [selectedCategories, setSelectedCategories] = useState<string[]>(note?.categories.map((c) => c.id) ?? []);
  ...
  const [footnoteDraft, setFootnoteDraft] = useState<FootnoteInput>({ mode: "text", book: "", volume: "", page: "", hadith_number: "", note_text: "" });
  ...
  function addFootnote() {
    const hasUsefulBook = footnoteDraft.mode === "book" && (footnoteDraft.book || footnoteDraft.note_text);
    const hasUsefulText = footnoteDraft.mode === "text" && footnoteDraft.note_text;
    if (!hasUsefulBook && !hasUsefulText) return;
    setFootnotes((prev) => [...prev, footnoteDraft]);
    setFootnoteDraft({ mode: footnoteDraft.mode, book: "", volume: "", page: "", hadith_number: "", note_text: "" });
  }

  function removeFootnote(index: number) {
    setFootnotes((prev) => prev.filter((_, i) => i !== index));
    setContentHtml((prev) => prev.replace(new RegExp(`<a href="#footnote-${index + 1}" id="footnote-ref-${index + 1}"><sup>\\[${index + 1}\\]<\\/sup><\\/a>`, "g"), ""));
  }
  ...
  return (
    <form action={onSubmit} className="card" style={{ padding: 24 }}>
      ...
      <div className="grid grid-3" style={{ marginBottom: 16 }}>
        <input className="input" placeholder="Not başlığı" value={title} onChange={(e) => setTitle(e.target.value)} />
        <input className="input" type="date" value={publishedDate} onChange={(e) => setPublishedDate(e.target.value)} />
      ...
      <div className="grid grid-2">
        <div>
            ...
            <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
              {categories.map((category) => (
                <button
                  ...
                  type="button"
                  className="btn"
                  style={{ background: selectedCategories.includes(category.id) ? "#0f172a" : "#f8fafc", color: selectedCategories.includes(category.id) ? "white" : "#334155" }}
            ...
            <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
              {tags.map((tag) => (
                <button
                  ...
                  type="button"
                  className="btn"
                  style={{ background: selectedTags.includes(tag.id) ? "#0f172a" : "#f8fafc", color: selectedTags.includes(tag.id) ? "white" : "#334155" }}
          ...
          <div className="card" style={{ padding: 14 }}>
            <div style={{ display: "flex", gap: 8, flexWrap: "wrap", marginBottom: 12 }}>
              <button type="button" className="btn" onClick={insertFootnoteRef}>Dipnot İşareti Ekle</button>
            </div>
            <textarea
              className="input"
        ...
        <div>
          ...
          <div className="card" style={{ padding: 14 }}>
            <select className="select" value={footnoteDraft.mode} onChange={(e) => setFootnoteDraft((prev) => ({ ...prev, mode: e.target.value as "text" | "book" }))}>
            ...
            {footnoteDraft.mode === "book" && (
              <div className="grid grid-2" style={{ marginTop: 12 }}>
                <input className="input" placeholder="Kitap" value={footnoteDraft.book ?? ""} onChange={(e) => setFootnoteDraft((prev) => ({ ...prev, book: e.target.value }))} />
                <input className="input" placeholder="Cilt" value={footnoteDraft.volume ?? ""} onChange={(e) => setFootnoteDraft((prev) => ({ ...prev, volume: e.target.value }))} />
                <input className="input" placeholder="Sayfa" value={footnoteDraft.page ?? ""} onChange={(e) => setFootnoteDraft((prev) => ({ ...prev, page: e.target.value }))} />
                <input className="input" placeholder="Hadis Numarası" value={footnoteDraft.hadith_number ?? ""} onChange={(e) => setFootnoteDraft((prev) => ({ ...prev, hadith_number: e.target.value }))} />
              </div>
            )}

            <textarea className="input" style={{ marginTop: 12, minHeight: 90 }} placeholder="Dipnot metni" value={footnoteDraft.note_text ?? ""} onChange={(e) => setFootnoteDraft((prev) => ({ ...prev, note_text: e.target.value }))} />
            <button type="button" className="btn" style={{ marginTop: 12 }} onClick={addFootnote}>Dipnotu Kaydet</button>

            <div style={{ display: "grid", gap: 12, marginTop: 14 }}>
              {orderedFootnotes.map((footnote, index) => (
                <div key={index} className="card" style={{ padding: 12, background: "#f8fafc" }}>
                  <div style={{ display: "flex", justifyContent: "space-between", gap: 12, marginBottom: 8 }}>
                    <strong>[{footnote.sort}] Dipnot</strong>
                    <button type="button" className="btn" onClick={() => removeFootnote(index)}>Sil</button>
                  </div>
      ...
      <div style={{ display: "flex", gap: 10, marginTop: 16 }}>
        <button type="submit" className="btn" onClick={() => setStatus("draft")}>Taslak Kaydet</button>
        <button type="submit" className="btn btn-green" onClick={() => setStatus("published")}><PlusSquare size={16} />Notu Yayımla</button>
      </div>
    </form>
  );
}
